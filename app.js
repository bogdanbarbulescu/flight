/**
 * Flight Simulator Application v4
 *
 * Fixes initialization order, route selector population,
 * and TypeError in updateInfoCard.
 */
document.addEventListener('DOMContentLoaded', () => {

    // --- Configuration & Constants ---
    const ROUTES_JSON_URL = 'routes.json';
    // ... (Constants: KTS_TO_KPH, FT_TO_M, etc. - remain the same) ...
    ...
    let simState = null; // Initialized in resetSimulationState
    let simulationTimer = null;
    let totalRouteDistance = 0; // Initialize to 0
    let flownDistance = 0;      // Initialize to 0
    ...
    async function init() {
        ...
        try {
            // 1. Load routes (essential first step)
            await loadRoutes();

            // 2. Setup foundational elements
            initDarkMode();
            setupMap();
            setupCharts();

            // 3. Populate UI & Add Listeners (DEPENDS ON routes loaded)
            setupUI(); // This now populates the selector using availableRoutes

            // 4. Set up the initial route and state (if routes loaded)
            if (selectedRouteIndex >= 0) {
                ...
                // Initial UI draw MUST happen after state is fully set by selectRoute->resetSimulationState
                if (simState) {
                    updateUI(simState, 0);
                ...
                // Disable controls as no route is available
                updateAutopilotButtonState(); // Call to set disabled states correctly
            }
            ...
            // Display error message (existing code is fine)
    ...
    async function loadRoutes() {
        ...
        availableRoutes = []; selectedRouteIndex = -1; // Reset state before fetch
        try {
            ...
            if(!data[0]?.origin?.coordinates || !data[0]?.destination?.coordinates) throw new Error("First route object has missing coordinate structure.");
            availableRoutes = data;
            selectedRouteIndex = 0; // Set default index only on SUCCESS
            ...
            availableRoutes = []; selectedRouteIndex = -1; // Ensure state reflects failure
    ...
    function selectRoute(index, isInitialLoad = false) {
        ...
        if (index < 0 || index >= availableRoutes.length) { console.error(`SELECTROUTE: Invalid index ${index}. Available: ${availableRoutes.length}`); if (routeSelector) routeSelector.value = selectedRouteIndex; return; }
        // Prevent redundant processing if index hasn't changed, *unless* it's initial load
        if (index === selectedRouteIndex && !isInitialLoad) {
            console.log(`SELECTROUTE: Re-selecting same route index ${index}. Resetting anyway.`);
             // Force reset even if index is the same (user clicked Reset or re-selected)
             // Reset simulation state FIRST
            resetSimulationState();
            // Then update map elements
            updateMapForRoute();
            // Then update UI
            if (simState) updateUI(simState, 0);
            else console.error("SELECTROUTE: simState is null after reset on re-selection!");
            return; // Stop here after resetting the same route
        }
        ...
        // 5. Update UI *after* state and map are fully reset/updated
        // This prevents TypeErrors from updateUI trying to use old/null state
        if (simState) {
            updateUI(simState, 0);
        } else {
            console.error("SELECTROUTE: simState is null after resetting for new route!");
        }

        // Ensure selector visually matches the selected index (especially for initial load)
    ...
    function resetSimulationState() {
         ...
         const currentRoute = availableRoutes[selectedRouteIndex];
         // More robust check for valid route data
         if (!currentRoute?.origin?.coordinates?.latitude || !currentRoute?.origin?.coordinates?.longitude ||
             !currentRoute?.destination?.coordinates?.latitude || !currentRoute?.destination?.coordinates?.longitude) {
         ...
         simState = {
             lat: originCoords[0], lon: originCoords[1], altitude: 50, // Start low (internal feet)
         ...
         if (altitudeSpeedChart?.data) { altitudeSpeedChart.data = { ...altitudeSpeedChart.data, labels: [], datasets: [{...altitudeSpeedChart.data.datasets[0], data:[]}, {...altitudeSpeedChart.data.datasets[1], data:[]}] }; altitudeSpeedChart.update('none'); }
         if (forcesChart?.data) { forcesChart.data = { ...forcesChart.data, labels: [], datasets: [{...forcesChart.data.datasets[0], data:[]}, {...forcesChart.data.datasets[1], data:[]}, {...forcesChart.data.datasets[2], data:[]}] }; forcesChart.update('none'); }
    ...
    function setupUI() {
        ...
        // Populate Route Selector (Ensure routes are loaded)
        if (routeSelector) {
            ...
            if (availableRoutes.length > 0 && selectedRouteIndex >= 0) {
                ...
                routeSelector.value = selectedRouteIndex; // Set initial value based on loaded data
    ...
    // --- Other Functions (Map, Charts, Simulation Loop, Physics, UI Updates, Helpers) ---
    // Keep the verified implementations from the previous response for:
    // updateRouteDisplay, initDarkMode, toggleDarkMode, updateChartTheme, setupMap, setupCharts,
    // updateMapForRoute, simulationLoop, updateAltitude, updateSpeed, updateHeading,
    // calculateWindEffect, updatePosition, updateFlownDistance, checkWaypointArrival,
    // checkLanding, updateUI, updatePFDDisplay, updateInfoCard, updateCharts,
    // toggleAutopilot, handleAltitudeSlider, handleSpeedSlider, updateAutopilotButtonState,
    // startSimulation, stopSimulation, degreesToRadians, radiansToDegrees, getDistance,
    // getBearing, calculateNewPosition, calculateTotalRouteDistance

    // (Paste the verified implementations of the above functions here)
    // ... (Ensure all functions listed above are included) ...
    // Example (paste all the functions from the previous good version):
    function updateRouteDisplay() { const currentRoute = availableRoutes[selectedRouteIndex]; if (!currentRoute) return; const originIata = currentRoute.origin.iata; const destIata = currentRoute.destination.iata; if (flightTitle) flightTitle.innerHTML = `${originIata} <i class="fa-solid fa-plane-departure"></i> ${destIata} Flight Simulator`; if (infoRouteOrigin) infoRouteOrigin.textContent = originIata; if (infoRouteDest) infoRouteDest.textContent = destIata; }
    // initDarkMode, toggleDarkMode, updateChartTheme...
    ...
    function resetSimulation() { console.log("UI: Reset Button Clicked"); selectRoute(selectedRouteIndex, false); }
